...
public class ComputeRanksLivy {
    ...
    public static void main(String[] args)
        ...
        SocialRankJob newJob = new SocialRankJob(d_max, i_max, 1000, false, debug);

        List<MyPair<String, Double>> noBacklinksResult = SparkJob.runJob(livy, newJob);
        ...
        try (PrintStream out = new PrintStream(new FileOutputStream("socialrank-livy-results.txt"))) {
            // list of node IDs for nodes in commons between backlinks and noBacklinks
            Set<String> noBackSet = new HashSet<>();
            for (MyPair<String, Double> item : noBacklinksResult) {
                noBackSet.add(item.getLeft());
            }

            Set<String> backSet = new HashSet<>();
            for (MyPair<String, Double> item : backlinksResult) {
                backSet.add(item.getLeft());
            }

            // list of nodes in backset that are also in nonackset
            List<MyPair<String, Double>> commonResults = backlinksResult.stream()
                    .filter(item -> noBackSet.contains(item.getLeft())).collect(Collectors.toList());
                
            String line = "";
            for (MyPair<String, Double> item : commonResults) {
                line += item.getLeft() + ",";
            }

            out.println(line.substring(0, line.length() - 1));

            // nodes in backset that are NOT in nobackset
            List<MyPair<String, Double>> backList = backlinksResult.stream()
                    .filter(item -> !noBackSet.contains(item.getLeft())).collect(Collectors.toList());
            // print all strings in comma separated lin in socialrank-livy-results.txt

            line = "";
            for (MyPair<String, Double> item : backList) {
                line += item.getLeft() + ",";
            }

            out.println(line.substring(0, line.length() - 1));

            List<MyPair<String, Double>> noBackList = noBacklinksResult.stream()
                    .filter(item -> !backSet.contains(item.getLeft())).collect(Collectors.toList());

            line = "";
            for (MyPair<String, Double> item : noBackList) {
                line += item.getLeft() + ",";
            }
